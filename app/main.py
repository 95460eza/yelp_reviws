...
def show_data(filename):
    ...
    def lemmatize_documents(docs):
        lemmatized_docs = []
        for doc in docs:
            spacy_doc = nlp(doc)
            lemmatized_docs.append(" ".join([token.lemma_ for token in spacy_doc]))
    ...
    df_sorted = df[~mask_outliers_in].sort_values(by=['Topic', 'Proba in Review'], ascending=[False, False])
